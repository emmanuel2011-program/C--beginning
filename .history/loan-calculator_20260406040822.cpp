...
// ---This is The Function to display the header making the code modular---
void displayHeader() {
    cout << "========================================" << endl;
...
// this function ensures that the user inputs a positive integer for the number of months, and it will keep prompting until a valid input is received. it serves as the security measure to prevent invalid inputs that could cause the program to crash or produce incorrect results.
int getValidInt(string prompt) {
    ...
    while (true) {  
        ...
        cin >> value;
        if (cin.fail() || value <= 0) {
            // clears the computer's memory of the bad input so it doesn't get stuck in a loop.
            cin.clear();
            cin.ignore(1000, '\n');
...
int main() {
    ...
    while (repeat == 'y' || repeat == 'Y') {
        ...
        int months = getValidInt("Enter Duration (Months): ");
         // mathematical formula to calculate the total interest and total payable amount based on the principal, annual interest rate, and duration in months. The formula used is a simple interest calculation where the total interest is calculated as (Principal * Annual Rate * Time) / 1200, and the total payable amount is the sum of the principal and total interest. Finally, the monthly payment is calculated by dividing the total payable amount by the number of months.
